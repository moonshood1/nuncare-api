    ...
    try {
        ...
        let doctors = await Doctor.find({
            ...
            $and: [
                ...
                { lat: { $lte: convertedLat + (radius / (111.32 * Math.cos(convertedLat * Math.PI / 180))) } },
                { lat: { $gte: convertedLat - (radius / (111.32 * Math.cos(convertedLat * Math.PI / 180))) } }
            ]
        }).sort({ createdAt: -1 });

        if (_.isEmpty(doctors)) {
            doctors = await Doctor.find({ _id: { $ne: user._id } }).sort({ createdAt: -1 });
        }
